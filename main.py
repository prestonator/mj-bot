...
def is_midjourney_image_message(message):
    return (
        str(message.author.id) == MID_JOURNEY_ID
        and len(message.attachments) > 0
        and message.attachments[0].url.endswith(".png")
    )
...
async def upscale_generated_image(
...
async def on_message(message):
    ...
    if "$mj_target" in message.content and message.content[0] == "$":
        try:
            ...
            target_info["targetHash"] = str(
                (message.reference.resolved.attachments[0].url.split("_")[-1]).split(
                    "."
                )[0]
            )
        except:
